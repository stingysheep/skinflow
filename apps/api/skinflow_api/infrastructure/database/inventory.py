    ...
    def list_grouped_assets(self) -> list[dict]:
        with self._lock:
            rows = self._connection.execute(
                ...
                "COALESCE(m.image_url,'') image_url, COUNT(*) total_quantity, "
                "SUM(CASE WHEN a.status='available' THEN 1 ELSE 0 END) available_quantity, "
                "SUM(CASE WHEN a.status='available' AND a.marketable=1 "
                "THEN 1 ELSE 0 END) marketable_quantity, "
                "SUM(CASE WHEN a.status='available' AND a.marketable=1 "
                "AND a.tradable=1 THEN 1 ELSE 0 END) tradable_quantity, "
                "CASE WHEN costs.quantity>0 THEN costs.invested / costs.quantity END average_cost "
                ",COALESCE(costs.quantity,0) held_quantity, "
                "CASE WHEN COUNT(DISTINCT NULLIF(a.wear_text,''))=1 THEN MAX(a.wear_text) "
                "WHEN COUNT(DISTINCT NULLIF(a.wear_text,''))>1 THEN '多种磨损' END wear_text "
                "FROM inventory_asset a LEFT JOIN item_metadata m "
                "ON m.market_hash_name=a.market_hash_name "
                "LEFT JOIN costs ON costs.market_hash_name=a.market_hash_name "
                "WHERE a.status='available' "
                "GROUP BY a.market_hash_name,localized_name,m.image_url "
                "ORDER BY localized_name,a.market_hash_name"
            ).fetchall()
        return [
            {
                **dict(row),
